...
void apply_transaction(int account_id, double amount) {
    int rc = pthread_mutex_lock(&accounts[account_id].lock);
    if (rc != 0) {
        fprintf(stderr, "Failed to acquire lock on account %d: %s\n", account_id, strerror(rc));
        return;
 }
double current = accounts[account_id].balance;
struct timespec ts = {0, 2000};
nanosleep(&ts, NULL);
accounts[account_id].balance = current + amount;
accounts[account_id].transaction_count++;

pthread_mutex_unlock(&accounts[account_id].lock);

    atomic_fetch_add(&expected_cents[account_id], (long long)llround(amount * 100.0));
}
...
int main(void) {
    ...
    for (int i = 0; i < NUM_ACCOUNTS; i++) {
        ...
        printf("Account %d: expected = %10.2f  actual = %10.2f  %s\n",
               i, expected, actual, mismatch ? "<-- MISMATCH (should not happen!)" : "(match)");
    }
